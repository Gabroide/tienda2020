...
if (isset($_GET["cat"]) && !empty($_GET["cat"])){
	...
	$query_DatosSEO = sprintf("SELECT * FROM tblcategoria WHERE strSEO_"._idiomaactual."=%s",
...
<!-- InstanceBeginEditable name="doctitle" -->
    <title><?php if (isset($_GET["cat"])) echo $row_DatosSEO["strNombre_"._idiomaactual]; ?><?php if (isset($_GET["marca"])) echo MostrarMarca($_GET["marca"]);?></title>
    <meta name="description" content="<?php if (isset($_GET["cat"])) echo $row_DatosSEO["strNombre_"._idiomaactual]; ?><?php if (isset($_GET["marca"])) echo MostrarMarca($_GET["marca"]);?>">
      ...
      <div class="col-sm-9 padding-right">
       ...
       <?php 
		  if (isset($_GET["id"]) && !empty($_GET["id"])){
			  ...
			  MostrarCategoriasDependientes($categoriaparaver);
		  }?>
      <?php 
		  if (isset($_GET["marca"]) && !empty($_GET["marca"])){
			  ?>
		...
		if ($totalRows_DatosConsulta > 0) {  
			...
			<div style="text-align: center">
				<div class="btn btn-default add-to-cart" id="show_more_main<?php echo $tutorial_id; ?>">
        			<span id="<?php echo $tutorial_id; ?>" class="show_more" title="Ver más productos"><?php echo _T17;?></span>
	...
	$(document).ready(function(){
		$(document).on('click','.show_more',function(){
			...
			$.ajax({
				...
				data:'id='+ID+'&max=<?php echo $resultadosporclick;?>'+'&cat=<?php echo $categoriaparaver.$bloqueparaAJAX;?>',
				<?php }?>
