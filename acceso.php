...
if (isset($_POST['strEmail'])) {
  ...
  //$MM_redirectLoginSuccess = "usuario.php?id=".$row_DatosConsulta["idUsuario"];
  ...
  if ($loginFoundUser) {
	...
	$_SESSION['tienda2017Front_UserId'] = mysqli_result($LoginRS,0,'idUsuario');
    $_SESSION['tienda2017Front_Mail'] = mysqli_result($LoginRS,0,'strEmail');
    $_SESSION['tienda2017Front_Nivel'] = mysqli_result($LoginRS,0,'intNivel');
	
	if (isset($_SESSION['MM2_Temporal'])){ 	
		ImportarCarritoTemporal($_SESSION['tienda2017Front_UserId']);
	}
	$_SESSION['MM2_Temporal']="ELEVADO";
    if (isset($_SESSION['PrevUrl']) && false) {
      $MM_redirectLoginSuccess = $_SESSION['PrevUrl'];	
    }
	//COMENTAR SOLO SI NO SE USA EL CHECK DE RECORDAR CONTRASEÑA, HABRÁ QUE USAR LA FUNCIÓN generar_cookie()

	if ((isset($_POST["remember"])) && ($_POST["remember"]=="1"))
	{
		generar_cookie($_SESSION['tienda2017Front_UserId']);
