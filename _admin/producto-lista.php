...
if (isset($_GET["valor"]))
{
    ...
    case 1:
        ...
        break;
    case 2:
        $parametro_orden= "strNombre_1";
        break;
    case 3:
        $parametro_orden= "dblPrecio";
        break;
    case 4:
        $parametro_orden= "intStock";
        break;
	case 5:
...
if ((isset($_GET["MM_Buscar"])) && ($_GET["MM_Buscar"]=="formbusqueda"))
{
	...
	$extramodelo=" strNombre_1 LIKE '%".$_GET["strBuscar"] ."%'";
	for($i=0; $i<$longitud; $i++)
	{
		$extramodelo.=" OR strNombre_1 LIKE '%".$porciones[$i] ."%'";
	}
	//(FIN BLOQUE NOMBRE)
	//(BLOQUE EMAIL)
	$porciones = explode(" ", $_GET["strBuscar"]);
	$longitud = count($porciones);
	$extramodelo.=" OR strDescripcion_1 LIKE '%".$_GET["strBuscar"] ."%'";
	for($i=0; $i<$longitud; $i++)
	{
		$extramodelo.=" OR strDescripcion_1 LIKE '%".$porciones[$i] ."%'";
	}
	//(FIN BLOQUE EMAIL)
...
<head>
    ...
    <title>Administración Tienda | Productos</title>
    <!-- InstanceEndEditable -->
    <!-- Bootstrap Core CSS -->
...
<div class="row">
	<div class="col-lg-5">
		<a href="producto-add.php" class="btn btn-outline btn-primary" title="Añadir un nuevo Producto">Añadir Producto</a>
	</div>
	<div class="col-lg-7">
	<div class="well">
...
<div class="row">
			 ...
			 do { 
				...
				<tr>
						...
						<?php if(($row_DatosConsulta["intStock"]==0) && ($row_DatosConsulta["intEstado"]==1)){?>
							<td style="background-color: red; font: bold; text-align: center">¡No hay stock!</td>
						<?php }
						else
						{?>		
							<td><?php echo $row_DatosConsulta["intStock"];?></td>
						<?php }?>
						<td><?php echo MostrarEstado($row_DatosConsulta["intEstado"]);?></td>
						<td><?php echo MostrarMarca($row_DatosConsulta["refMarca"]);?></td>
					<td><a title="Editar Producto" href="producto-edit.php?id=<?php echo $row_DatosConsulta["idProducto"];?>" class="btn btn-warning btn-circle"><i class="fa fa-edit"></i></a> <a title="Editar Opciones <?php echo $row_DatosConsulta["strNombre_1"];?>" href="productoopcion-edit.php?id=<?php echo $row_DatosConsulta["idProducto"];?>" class="btn btn-warning btn-circle"><i class="fa fa-circle-o "></i></a>  <a title="Editar Características <?php echo $row_DatosConsulta["strNombre_1"];?>" href="productocaracteristica-edit.php?id=<?php echo $row_DatosConsulta["idProducto"];?>" class="btn btn-warning btn-circle"><i class="fa fa-road "></i></a> <a title="Estadísticas <?php echo $row_DatosConsulta["strNombre_1"];?>" href="producto-stats.php?id=<?php echo $row_DatosConsulta["idProducto"];?>" class="btn btn-success btn-circle"><i class="fa fa-bar-chart-o  "></i></a> <a title="Duplicar Producto <?php echo $row_DatosConsulta["strNombre_1"];?>" href="producto-duplicar.php?id=<?php echo $row_DatosConsulta["idProducto"];?>" class="btn btn-info btn-circle"><i class="fa fa-gears"></i></a></td>
				</tr>
