...
if (isset($_GET["valor"]))
{
	switch ($_GET["valor"]) {
    case 1:
        $parametro_orden= "idReserva";
        break;
    case 2:
        $parametro_orden= "idUsuario";
        break;
    case 3:
        $parametro_orden= "fchReserva";
        break;
    case 4:
	...
	$parametro_orden= "idReserva"; //POR DEFECTO
...
if ((isset($_GET["MM_Buscar"])) && ($_GET["MM_Buscar"]=="formbusqueda"))
{
	...
	$porciones = explode(" ", $_GET["strBuscar"]);
	$longitud = count($porciones);
	$extramodelo=" fchReserva >= '".DateToQuotedMySQLDate($_GET["FDesde"]) ."' AND fchReserva <= '".DateToQuotedMySQLDate($_GET["FHasta"])."'";
	//(FIN BLOQUE FECHAS)

	
	$query_DatosConsulta = "SELECT * FROM tblreserva WHERE (".$extramodelo.") AND intEstado=".$_GET["ver"].$consultaextendida.$cadenaOrden;
	//echo $query_DatosConsulta;
}
else
{
	$query_DatosConsulta = sprintf("SELECT * FROM tblreserva WHERE intEstado=".$_GET["ver"].$cadenaOrden);
	}
...
<head>
    ...
    <title>Administración Tienda | Reservas</title>
    <!-- InstanceEndEditable -->
    <!-- Bootstrap Core CSS -->
  ...
  <div id="page-wrapper">
     <div class="row">
			...
			<div class="row">	
				<div class="col-lg-6">
					<div class="well"
						<form action="reserva-lista.php" method="get" id="formbusqueda" name="formbusqueda" role="form">
							<div class="row">
								<div class="col-lg-4">
								</div>
								<div class="col-sm-3">
									<div class="form-group">
										<div class='input-group date' >
											<label for="FDesde" style="display: none">Desde</label>
								...
								<div class="col-sm-3">
									<div class="form-group">
										<div class='input-group date' >
											<label for="FHasta" style="display: none">Hasta</label>
			...
			<div class="row">
			 ...
			 do { 
				...
				<tr>
						<td><?php echo $row_DatosConsulta["idReserva"];?></td>						
						<td>(Id: <?php echo $row_DatosConsulta["idUsuario"];?>) <?php echo ObtenerNombreUsuario( $row_DatosConsulta["idUsuario"])?></td>
						<td><?php echo DateToHumano($row_DatosConsulta["fchReserva"]);?> <?php echo HoraToHumano($row_DatosConsulta["fchReserva"]);?></td>
						<td><?php echo MostrarEstadoPedido($row_DatosConsulta["intEstado"]);?></td>
						<td><a href="reserva-edit.php?id=<?php echo $row_DatosConsulta["idReserva"];?>" class="btn btn-warning btn-circle" title="Tramitar Reserva <?php echo $row_DatosConsulta["idReserva"];?>: <?php echo ObtenerNombreUsuario( $row_DatosConsulta["idUsuario"])?>"><i class="fa fa-edit"></i></a></td>
				</tr>
